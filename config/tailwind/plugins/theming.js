...
// -----------------------------------------------------------------------------------------------------
// @ ARTISIAN TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
    ...
    ({ addComponents, e, theme }) => {
            ...
            _.fromPairs(
              _.flatten(
                _.map(
                  ...
                  (value, key) => [
                    [`--artisian-${e(key)}`, value],
                    [`--artisian-${e(key)}-rgb`, chroma(value).rgb().join(',')],
                  ]
                )
              )
            ),
      ...
      const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
        ...
        const background = theme(
          `artisian.customProps.background.${colorScheme}`
        );
        const foreground = theme(
          `artisian.customProps.foreground.${colorScheme}`
        );
          ...
          colors: _.fromPairs(
            _.flatten(
              _.map(
                ...
                (name) => [
                  [name, `rgba(var(--artisian-${name}-rgb), <alpha-value>)`],
                  [
                    `on-${name}`,
                    `rgba(var(--artisian-on-${name}-rgb), <alpha-value>)`,
                  ],
                ]
              )
            )
